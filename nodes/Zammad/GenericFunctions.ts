...
export async function zammadApiRequest(
	...
	if (authentication === 'basicAuth') {
		const credentials =
			await this.getCredentials<Zammad.BasicAuthCredentials>('zammad2BasicAuthApi');

		const baseUrl = tolerateTrailingSlash(credentials.baseUrl);

		options.url = `${baseUrl}/api/v1${endpoint}`;
		...
		options.skipSslCertificateValidation = credentials.allowUnauthorizedCerts;
	} else {
		const credentials =
			await this.getCredentials<Zammad.TokenAuthCredentials>('zammad2TokenAuthApi');
